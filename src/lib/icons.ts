 ...
 * Each icon is a 24×24 viewBox; uses currentColor for stroke.
 */
...
export function icon(name: keyof typeof PATHS | string, opts: IconOptions = {}): string {
  ...
  const aria = opts.ariaLabel
    ...
    : ' aria-hidden="true"';
  return `<svg${cls}${aria} xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
}
